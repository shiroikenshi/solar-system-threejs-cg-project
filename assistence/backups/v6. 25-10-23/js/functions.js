...
function onDocumentMouseDown(event) {
    ...
    if (intersects.length === 0) {
        ...
        secondRaycaster.params.Points.threshold = 10; // Valor de threshold diferente
...
function updateOpacity(orbitLine, marker, object) {
    ...
    // Ajuste esses valores conforme necessário para controlar a transição de opacidade
    const minDistance = 5 * (object.geometry.parameters.radius * 5); // Define a distância mínima para 100% de opacidade com base no raio do objeto
    const maxDistance = 50 * (object.geometry.parameters.radius * 5); // Define a distância máxima para 0% de opacidade com base no raio do objeto

    // Interpola a opacidade entre 0 e 1 com base na distância da câmera
    function linearInterpolation(value, min, max) {
        if (value <= min) return 0;
        if (value >= max) return 1;
        return (value - min) / (max - min);
    }

    const opacity = linearInterpolation(distanceToCamera, minDistance, maxDistance);
