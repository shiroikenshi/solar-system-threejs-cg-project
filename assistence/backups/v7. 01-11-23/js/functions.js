...
function updateCameraPositionY(minCameraY, maxCameraY) {
...
function moverCameraParaExplorar(cena) {
    ...
    setTimeout(() => {
        ...
        // Calcula a nova posição da câmera com base no raio da esfera
        const distance = 1;
        const objectPosition = cena.position.clone();
        const direction = new THREE.Vector3().subVectors(camera.position, objectPosition).normalize();
        const newPosition = objectPosition.clone().add(direction.multiplyScalar(distance));
...
function loadingScreen(){
    ...
    }, 2000); // Inicia após 1700 milissegundos (2000 = 2500 - 500 para garantir a sincronia)

    // Oculte a tela de carregamento após a animação de desaparecimento
    setTimeout(() => {
        document.getElementById('loading-screen').style.display = 'none';
    }, 2500); // Ajuste o tempo de espera conforme necessário
}